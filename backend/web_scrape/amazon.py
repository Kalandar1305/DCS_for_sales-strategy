...
extractorAmazon =  selectorlib.Extractor.from_yaml_file('web_scrape/selectors/selectors_amazon.yml')
def parseTime(raw):
    s = raw.split()[-3:]
...
def parseAmazonReview(raw):
    try: 
        ...
        for r in raw['reviews']:
            ...
            r['likes'] = int('1' if likes == 'One' else likes.replace(",",""))
...
def getAmazonReviews(url, details = False, no_of_reviews=50):
    try:
        ...
        print("amazon:", no_of_reviews, end=" | pages: 1 ")
        if no_of_reviews > 0:
